...
// Yahoo symbol for XETR: VVSM is typically VVSM.DE
...
async function fetchEURUSD() {
  ...
  const json = await res.json();
  const result = json.chart?.result?.[0];
  if (!result) throw new Error("No data for EURUSD=X");
...
async function run() {
  try {
    ...
    const latestValueEur = SHARES * latestPrice * eurusd;
    ...
    const output = {
      ...
      latestPrice,
      latestValueEur,
      timestamps: etf.timestamps, // UNIX seconds
      closes: etf.closes
    };
    ...
    console.log(
      `Latest VVSM: $${latestPrice.toFixed(2)}, EURUSD: ${eurusd.toFixed(4)}, Position: €${latestValueEur.toFixed(2)}`
    );
  } catch (err) {
